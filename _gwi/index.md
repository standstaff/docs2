...
* Features are not provided at this time. Morpheme information can be found for many words in the MISC column.
* However, each VERB takes one of four voice/valency markers known as its _classifier_. The proposed language-specific feature _Classifier_ has the values _Ø_, _L_, _Ł_, or _D_, consistent with the Athabascan literature. It is important to note that this use of the word _classifier_ is different than for Chinese.
...
* Postpositions ADP must be inflected for person and number with human objects. If there is an areal object, the postposition takes the prefix _gwï-_. The feature-value -Areal=Yes_ may be needed for this. 
